...
let tentativi = [] //variabile per i numeri di tentativi
containerEl.innerHTML = `<div class='box_iniziale border-1 border border-dark')">Seleziona una difficoltà e premi start</div>`;
function Start(){
    tentativi = [] //reset
    Field = [] //reset
    perso = false //reset
    ...
    switch(difficult){
        ...
        case "Select":
            ...
            return //Se non si seleziona una difficoltà esce dalla funzione
...
function clickEvent(id) {
    ...
    if(terminato == false){
        if (Field[id-1] == "Bomba"){
            ...
            box.style.color = "red";
            if(perso == false){
                alert("Mi dispiace hai perso con: "+tentativi.length+" tentativi") 
                perso = true;
                Loser()
                terminato = true
            }
